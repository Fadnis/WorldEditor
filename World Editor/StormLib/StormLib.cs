...
namespace World_Editor.StormLib
{
    ...
    [Flags]
    public enum CreateArchiveFlags : uint
    {
    ...
    public class StormLib
    {
        ...
        [DllImport("StormLib.dll")]
        public static extern bool SFileCreateArchive([MarshalAs(UnmanagedType.LPStr)] string szMpqName, [MarshalAs(UnmanagedType.U4)] CreateArchiveFlags dwFlags,
            uint dwMaxFileCount, IntPtr phMpq);

        [DllImport("StormLib.dll")]
        public static extern bool SFileAddFileEx(IntPtr hMpq, [MarshalAs(UnmanagedType.LPStr)] string szFileName, [MarshalAs(UnmanagedType.LPStr)] string szArchivedName,
    ...
    public class MpqArchive : IDisposable
    {
        ...
        public MpqArchive(string file, CreateArchiveFlags Flags)
        {
            bool r = Create(file, CreateArchiveFlags.MPQ_CREATE_ARCHIVE_V2);
        }
        ...
        private unsafe bool Create(string file, CreateArchiveFlags Flags)
        {
            IntPtr h;
            IntPtr hp = (IntPtr)(&h);
            bool r = StormLib.SFileCreateArchive(file, Flags, 0x80000, hp);
            if (r)
            {
                handle = h;
            }
        ...
        public bool AddFileEx(string from, string to)
        {
            return StormLib.SFileAddFileEx(handle, from, to, 0, 0, 0);
        }
    ...
    public class MpqFile : IDisposable
        {
            ...
            public unsafe MemoryStream ToStream()
            {
                ...
                MemoryStream stream = new MemoryStream(50000);
                ...
                if (dwBytes != szBuffer.Length)
                    throw new IOException(String.Format("Can't read file to stream"));

                if (dwBytes > 0)
                    stream.Write(szBuffer, 0, dwBytes);
