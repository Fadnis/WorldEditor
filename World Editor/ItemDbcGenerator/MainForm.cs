...
namespace World_Editor.ItemDbcGenerator
{
    public partial class MainForm : Form
    {
        ...
        private void btnGenerate_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ItemTemplate[] Items = null;
                if (checkAll.Checked)
                {
                    Items = ProjectManager.Connection.GetItemTemplate();
                }
                else
                {
                    Items = ProjectManager.Connection.GetItemTemplate(Misc.ParseToUInt(txtFrom.Text), Misc.ParseToUInt(txtTo.Text));
                }
